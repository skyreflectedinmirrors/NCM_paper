...
In a \textit{deep}-SIMD vectorization, a vector-unit cooperates to evaluate chemical source terms for a single thermo-chemical state $j$, thus SIMD-lanes loading species concentrations $k, k+1\ldots$ for e.g., state 1, will access sequential memory locations if the data is stored in ``C''-order.
...
\begin{equation}
\text{L}_{ij,\text{lower}} = 2^{-p_{ij}} \le \left\lvert 1 - \frac{R_{ij}^{\prime}}{R_{ij}^{\prime\prime}} \right\rvert \le 2^{-q_{ij}} = \text{L}_{ij,\text{upper}}
\label{e:bounds}
\end{equation}
with $q_{ij} \le p_{ij}$.
...
Finally, when determining the error in the net ROP between this code and Cantera~\cite{Cantera}, the bounds on the percent error attributable to precision loss, $\text{P}_{ij,\text{err}}$ can be estimated as:
\begin{equation}
...
\multicolumn{1}{l}{\ce{iC5H11OH}\cite{Sarathy:2013jr}} & 6.80e-8 &  1.48e-7 & 5.26e-1 & 4.30e-1 & 2.26e00 & 6.10e+01 \\
\bottomrule
\end{tabular}
