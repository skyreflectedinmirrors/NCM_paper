...
A code generation platform for single-instruction, multiple-data vectorized OpenCL chemical source term evaluation was developed and validated against Cantera for a wide range of chemical kinetic models.
Speedups of \SI{1.88}{2.42}{$\times$} were observed over a baseline single-instruction, multiple-thread parallelized code showing a clear performance benefit to use of a row-major data layout.
...
\begin{keyword}
    Chemical Kinetics\sep Single Instruction, Multiple Data\sep Vector Processing
\end{keyword}
...
The scaling efficiency for decreases with the number of cores used for all models; interestingly the efficiency for the hydrogen model decreases fastest, while the larger models have similar efficiencies with isopentanol having the highest efficiency in all cases.
