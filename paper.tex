...
\label{e:rel_err}
\end{equation}
For the reaction $i$ and state $j$ that results in the maximum ROP error $E_{R}$, the lower precision bound is set to $\text{P}_{\text{lower}}^{*} = \text{P}_{ij,\text{lower}}$.
...
We see here that the estimated lower bound on loss of precision is \SIrange{4}{40}{$\times$} the error in net ROP for all but the \ce{iC5H11OH} model, explaining the large increase in the net ROP error norm as compared to either the forward or backward ROP error norms.
