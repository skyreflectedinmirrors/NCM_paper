...
The error of a quantity $X_{i,j}$ for state $j$, and index $i$ was determined as:
\begin{equation}
 E_{X_{i}} = \left\lVert \left\lVert \frac{\left\lvert X_{i,j,\text{CT}} - X_{i,j}\right\rvert}{\num{e-10} + \num{e-6} * \left\lvert X_{i,j,\text{CT}} \right\rvert} \right\rVert_{j,\infty} \right\rVert_{i, \infty}
\label{e:rel_err}
\end{equation}
Finally, to estimate the effect of loss of precision on the net rates of progress, we let $P_{\text{min}}^{*} = P_{i,\text{min}}$ for the $i,j$ value that results in $E_{R_{i}}$.
...
\toprule
\multicolumn{1}{c}{Model} & \multicolumn{1}{c}{$E_{R_{i}^{\prime}}$} & \multicolumn{1}{c}{$E_{R_{i}^{\prime\prime}}$} &\multicolumn{1}{c}{$E_{R_{i}}$} & \multicolumn{1}{c}{$\text{P}_{\text{min}}^{*}$} & \multicolumn{1}{c}{$E_{\frac{\text{d}[C_k]}{\text{d}t}}$} & \multicolumn{1}{c}{$E_{\frac{\text{d}[T]}{\text{d}t}}$} \\
\midrule
