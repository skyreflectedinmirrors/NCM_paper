...
In the ``F'' (Fortran column-major) format, the concentrations of a single species $k$ over all $N$ thermo-chemical states are adjacent: $[C_k]_j, [C_k]_{j + 1}\ldots[C_k]_{N}$;
conversely in the ``C'' (C row-major) format, the species concentrations for the $j$-th thermo-chemical state: $[C_k]_j, [C_{k+1}]_j\ldots$ are stored sequentially in memory;
...
In this work a \textit{shallow}-SIMD vectorization, in which each SIMD-lane evaluates chemical source terms for a different thermo-chemical state, has been implemented.
If the data is stored in ``F''-order, the SIMD-lanes will access sequential locations in memory however the next species' concentration---for example---will be $N$ memory locations away, potentially resulting in more cache-misses on the CPU.
In future efforts, a so called \textit{deep}-SIMD vectorization will be pursued, in which a vector-unit cooperates to evaluate chemical source terms for a single thermo-chemical state.
In this context, a deep-vectorization may result in SIMD-\textit{waste}---similar to thread-divergence in GPU-processing---caused by different SIMD-lanes executing different instructions (e.g. from differing for-loop bounds).
However, SIMD-lanes in a ``C''-ordered deep-vectorization access sequential memory locations and further the data locality of e.g., species concentrations, will likely result in improved CPU cache-hit rates.
Past studies additionally demonstrate promising results for deep-SIMT vectorization on GPUs~\cite{Sewerin20151375}.
