...
\todo[inline]{Find a source for this?  It all seems to be weird cpu journalism sites, maybe search intel}
...
SIMT-vectorized chemical source term evaluation was \SI{13}{\percent} and \SI{35}{\percent} slower than the equivalent OpenMP code on the CPU and MIC respectively.
The SIMD-vectorized chemical source terms were up to \SI{3}{$\times$} faster on both platforms than the OpenMP baseline; GPU chemical source term evaluation was of similar performance to the SIMD-vectorized versions.
SIMD-vectorization improved the integrator performance over the OpenMP baseline by \SIrange{2.5}{2.8}{$\times$} on the CPU and \SIrange{4.7}{4.9}{$\times$} on the MIC.
...
\todo[inline]{and OpenMp, time permitting}
...
In Sec.~\ref{results} the effect of this ordering will be shown to have a large performance impact on the program runtime.
